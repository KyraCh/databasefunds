    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($end_date > $now) {
            if ($my_bid < $current_price) {
                print_bids_li_red($item_id, $title, $my_bid, $description, $current_price, $num_bids, $end_date,$images);

            }
            else {
                //if auction is indeed active, print bid
                print_bids_li($item_id, $title, $my_bid, $description, $current_price, $num_bids, $end_date,$images);
            }
        }
    }
}
